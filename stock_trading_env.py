...
class StockTradingEnv(gym.Env):
    def __init__(self, df, initial_balance=10000, max_shares=10, transaction_fee_percent=0.001):
        ...
        try:
            ...
            def safe_numeric_convert(x):
                if isinstance(x, str):
                    x = x.replace(',', '').replace('%', '')  # Remove commas and '%'
                    return float(x) if x else 0.0  # Convert to float, handle empty strings
                return float(x)
            ...
            # Convert Percent Change column separately (keep sign, remove '%')
            # Convert Percent Change column safely
            if 'Percent Change' in df.columns:
                df['Percent Change'] = df['Percent Change'].str.replace('%', '', regex=True)  # Remove '%'
                df['Percent Change'] = df['Percent Change'].str.strip()  # Remove leading/trailing spaces
                df['Percent Change'] = df['Percent Change'].replace({'-': None, '': None})  # Replace invalid values with NaN
                df['Percent Change'] = pd.to_numeric(df['Percent Change'], errors='coerce')  # Convert to float safely
        ...
        self.df['MACD'] = ta.trend.MACD(self.df['Close']).macd().fillna(0)
        self.df['Signal'] = ta.trend.MACD(self.df['Close']).macd_signal().fillna(0)
        self.df['RSI'] = ta.momentum.RSIIndicator(self.df['Close']).rsi().fillna(50)
        ...
        self.df['EMA_20'] = ta.trend.EMAIndicator(self.df['Close'], window=20).ema_indicator().fillna(0)
        
        # Add volatility measures
        self.df['Volatility'] = self.df['Returns'].rolling(window=20).std().fillna(0)
        self.df['ATR'] = ta.volatility.AverageTrueRange(self.df['High'], self.df['Low'], self.df['Close']).average_true_range().fillna(0)
        
        # Volume indicators
        self.df['Volume_SMA'] = self.df['Volume'].rolling(window=20).mean().fillna(0)
        self.df['Volume_Ratio'] = (self.df['Volume'] / self.df['Volume_SMA']).fillna(1)
        
        # Momentum indicators
        self.df['MFI'] = ta.volume.MFIIndicator(self.df['High'], self.df['Low'], self.df['Close'], self.df['Volume']).money_flow_index().fillna(50)
    ...
    def step(self, action):
        ...
        next_price = float(self.df.iloc[min(self.current_step + 1, len(self.df)-1)]['Close'])
